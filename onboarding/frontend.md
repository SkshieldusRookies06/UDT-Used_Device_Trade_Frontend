# 내 하루 루프 — 프론트 (모든 명령은 `safedeal-frontend/` 루트에서 시작)

> **이 프로젝트는 리포가 둘이다.** 백엔드 리포를 **형제 폴더**로 클론해 둔다 —
> `npm run gate`·`npm run mock`이 `../safedeal-backend`를 찾는다.
...
cd <작업폴더>
git clone <백엔드 URL> safedeal-backend
git clone <프론트 URL> safedeal-frontend
cd safedeal-frontend
...
```
1) 터미널 1 — 백엔드 기동
   (cd ../safedeal-backend && ./mvnw spring-boot:run -Dspring-boot.run.profiles=local)
...
| `Cannot find module` (게이트) | 프론트 리포 루트가 아닌 곳에서 쳤다. `cd safedeal-frontend` |
